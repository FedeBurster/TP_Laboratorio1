...
/** \brief Lee el archivo y guarda lo leido en la variable ingresada
 *
 * \param id int* variable donde se guarda lo leido
 * \return Devuelve 1 si falla, 0 si carga el id correctamente
 *
 */
int obtenerId(int* id)
{
    int error = 1;
    FILE* archivoId=fopen("proximoId.bin","rb");
    if(archivoId!=NULL)
    {
...
/** \brief Escribe en un archivo el dato ingresado
 *
 * \param id int dato a escribir
 * \return Devuelve 1 si falla, 0 si se guarda correctamente
 *
 */
int actualizarId(int id)
{
    int error=1;
    id++;
    FILE* archivoId=fopen("proximoId.bin","wb");
    if(archivoId!=NULL)
    {
