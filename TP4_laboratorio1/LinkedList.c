...
void* ll_get(LinkedList* this, int index)
{
    ...
    if(this!=NULL && auxNode!=NULL && index<ll_len(this))
    {
        if(index==0)
        {
...
LinkedList* ll_subList(LinkedList* this,int from,int to)
{
    ...
    if (this != NULL && (from >= 0 && from < ll_len(this) ) && (to > from && to <= ll_len(this)) )
    {
        ...
        if(cloneArray != NULL && element !=NULL )
        {
            for(i=from; i<to; i++)
            {
                element = getNode(this, i);
                ll_add(cloneArray, element->pElement);
            }
        }
    }
    return cloneArray;
...
int ll_sort(LinkedList* this, int (*pFunc)(void* ,void*), int order)
{
    ...
    if(this!=NULL && (order==0 || order==1) && ll_len(this)>0 && pFunc!=NULL)
    {
        ...
        {
            if(getNode(this, i) != NULL && auxNode != NULL)
            {
                for(j=i+1;j<ll_len(this);j++)
                {
                    if(getNode(this, j) != NULL)
                    {
                        ...
                        {
                            auxNode->pElement = getNode(this, i)->pElement;
                            getNode(this, i)->pElement = getNode(this, j)->pElement;
                            getNode(this, j)->pElement = auxNode->pElement;
